...
namespace std
{
	...
	struct atomic_flag {
	private:
		explicit atomic_flag(atomic_flag const&);
		explicit atomic_flag(atomic_flag&&);
		atomic_flag& operator=(atomic_flag const&);
		atomic_flag& operator=(atomic_flag&&);
	
	public:
		atomic_flag() : val(false) { }
		explicit atomic_flag(bool v) : val(v) { }
		
		atomic_flag& operator=(bool v) {
		...
		{
			return val.exchange(true, order, d);
		}
