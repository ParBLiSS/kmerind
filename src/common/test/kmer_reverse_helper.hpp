...
namespace bliss
{
  ...
  class KmerReverseHelper
  {
   ...
   protected:
    ...
    static constexpr uint8_t BLISS_ALIGNED_ARRAY(simd_mask_b, 16, 16) = {0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F,0x0F};

    // mask for reversing the order of items
    static constexpr uint8_t BLISS_ALIGNED_ARRAY(simd_rev_mask_b, 16, 16) = {0x0F,0x0E,0x0D,0x0C,0x0B,0x0A,0x09,0x08,0x07,0x06,0x05,0x04,0x03,0x02,0x01,0x00};

    // lookup table for reversing bits in a byte in groups of 1
    static constexpr uint8_t BLISS_ALIGNED_ARRAY(simd_lut1_lo_b, 16, 16) = {0x00,0x08,0x04,0x0c,0x02,0x0a,0x06,0x0e,0x01,0x09,0x05,0x0d,0x03,0x0b,0x07,0x0f};

    static constexpr uint8_t BLISS_ALIGNED_ARRAY(simd_lut1_hi_b, 16, 16) = {0x00,0x80,0x40,0xc0,0x20,0xa0,0x60,0xe0,0x10,0x90,0x50,0xd0,0x30,0xb0,0x70,0xf0};

    // lookup table for reversing bits in a byte in groups of 2
    static constexpr uint8_t BLISS_ALIGNED_ARRAY(simd_lut2_lo_b, 16, 16) = {0x00,0x04,0x08,0x0c,0x01,0x05,0x09,0x0d,0x02,0x06,0x0a,0x0e,0x03,0x07,0x0b,0x0f};

    static constexpr uint8_t BLISS_ALIGNED_ARRAY(simd_lut2_hi_b, 16, 16) = {0x00,0x40,0x80,0xc0,0x10,0x50,0x90,0xd0,0x20,0x60,0xa0,0xe0,0x30,0x70,0xb0,0xf0};
...
//          print(in , "in");
          ...
          memcpy(result.getDataRef(), tmp, Kmer::nWords * sizeof(WORD_TYPE));                                             // SSE2
          ...
          // store back out
          _mm_storeu_si128((__m128i*)(result.getDataRef()), mword);                                                     // SSE2
        }
      ...
      if (simd_iters > 0) {
        auto in = src.getData();
        auto out = result.getDataRef() + Kmer::nWords - simd_stride;
...
//
      ...
      result.template right_shift_bits<(Kmer::nWords * sizeof(WORD_TYPE) * 8 - Kmer::nBits)>();
...
//          print(in , "in");
    ...
    inline Kmer reverse_swar(Kmer const & src) const
    {
      ...
      for (int i = 0, j = Kmer::nWords - 1, max = Kmer::nWords; i < max; ++i, --j)
        result.getDataRef()[j] = word_reverse<A, WORD_TYPE>(src.getData()[i]);
...
//      std::cout << "SWAR before: " << src << std::endl;
    ...
    {
      Kmer result(false);  // empty kmerfor output

      // swap the word order.  also do rev_comp for the chars in the word at the same time.
      for (int i = 0, j = Kmer::nWords - 1, max = Kmer::nWords; i < max; ++i, --j)
        result.getDataRef()[j] = word_reverse_complement<A>(src.getData()[i]);

      // shift if necessary      // ununsed bits will be set to 0 by shift
//      printf("HELPER before shift: %s\n", result.toAlphabetString().c_str());
//      // shift if necessary      // ununsed bits will be set to 0 by shift
//      printf("HELPER: right shift by %lu\n", (Kmer::nWords * sizeof(WORD_TYPE) * 8 - Kmer::nBits));
      result.template right_shift_bits<(Kmer::nWords * sizeof(WORD_TYPE) * 8 - Kmer::nBits)>();
...
//    static constexpr uint8_t  mask_8[3]  = { 0x0F, 0x33, 0x55 };
    ...
    inline Kmer reverse_bswap(Kmer const & src) const
    {
      ...
      // unchunked reverse works.
            for (int i = 0, j = Kmer::nWords - 1, max = Kmer::nWords; i < max; ++i, --j)
              result.getDataRef()[j] = word_reverse_bswap<A, WORD_TYPE>(src.getData()[i]);
...
/*      //== chunked revcomp does NOT always work, although slightly faster, especially for small words.
      ...
      //      printf("1st step:\t\t%s\n", result.toAlphabetString().c_str());
            ...
            if (rem > 1) {
              ...
              memcpy(result.getDataRef(), &mword, rem * sizeof(WORD_TYPE));
            } else if (rem == 1) {
              result.getDataRef()[0] = word_reverse_bswap<A, WORD_TYPE>(src.getData()[Kmer::nWords - 1]);
            }  // else no rem.
      //      printf("2nd step:\t\t%s\n", result.toAlphabetString().c_str());
*/
    ...
    {
      Kmer result(false);  // empty kmer for output


      // swap the word order.  also do rev_comp for the chars in the word at the same time.
      for (int i = 0, j = Kmer::nWords - 1, max = Kmer::nWords; i < max; ++i, --j)
        result.getDataRef()[j] = word_reverse_complement_bswap<A, WORD_TYPE>(src.getData()[i]);
...
//      printf("1st step:\t\t%s\n", result.toAlphabetString().c_str());
      ...
      if (rem > 1) {
        ...
        result.getDataRef()[0] = word_reverse_complement_bswap<A, WORD_TYPE>(src.getData()[Kmer::nWords - 1]);
      }  // else no rem.
//      printf("2nd step:\t\t%s\n", result.toAlphabetString().c_str());
*/
//      printf("HELPER before shift: %s\n", result.toAlphabetString().c_str());
...
//      printf("HELPER: right shift by %lu\n", (Kmer::nWords * sizeof(WORD_TYPE) * 8 - Kmer::nBits));
  ...
  template<typename Kmer>
  constexpr uint8_t KmerReverseHelper<Kmer>::BLISS_ALIGNED_ARRAY(simd_mask_b, 16, 16);
  template<typename Kmer>
  constexpr uint8_t KmerReverseHelper<Kmer>::BLISS_ALIGNED_ARRAY(simd_rev_mask_b, 16, 16);
  template<typename Kmer>
  constexpr uint8_t KmerReverseHelper<Kmer>::BLISS_ALIGNED_ARRAY(simd_lut1_lo_b, 16, 16);
  template<typename Kmer>
  constexpr uint8_t KmerReverseHelper<Kmer>::BLISS_ALIGNED_ARRAY(simd_lut1_hi_b, 16, 16);
  template<typename Kmer>
  constexpr uint8_t KmerReverseHelper<Kmer>::BLISS_ALIGNED_ARRAY(simd_lut2_lo_b, 16, 16);
  template<typename Kmer>
  constexpr uint8_t KmerReverseHelper<Kmer>::BLISS_ALIGNED_ARRAY(simd_lut2_hi_b, 16, 16);
