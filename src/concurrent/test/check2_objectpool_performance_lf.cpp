...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i <= 8; ++i) {
    ...
    timeAppendMultipleBuffers<bliss::concurrent::ObjectPool<bliss::io::Buffer<bliss::concurrent::LockType::LOCKFREE, 8192>, lt, false > >(1, count, lt, bliss::concurrent::LockType::LOCKFREE, 8192);
...
//    INFOF("  time: entries %d nthread %d time %f\n", count, 1, time_span.count());
  ...
  for (int i = 1; i <= 8; ++i) {
    t1 = std::chrono::high_resolution_clock::now();
    timeAppendMultipleBuffers<bliss::concurrent::ObjectPool<bliss::io::Buffer<bliss::concurrent::LockType::LOCKFREE, 8192>, lt, false > >(i, count, lt, bliss::concurrent::LockType::LOCKFREE, 8192);
    t2 = std::chrono::high_resolution_clock::now();
    time_span = std::chrono::duration_cast<std::chrono::duration<double>>(t2 - t1);
//    INFOF("  time: entries %d nthread %d time %f\n", count, 1, time_span.count());
  }
