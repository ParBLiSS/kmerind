...
//  auto stats = ::mxx2::reduce::reduce(u, [](::std::tuple<T, T, T, T> const &x, ::std::tuple<T, T, T, T> const &y) {
...
#define TIMER_REPORT_MPI(timing, rank, comm) \
        do { \
          ...
          MPI_Comm_size(comm, &p); \
          \
          auto dur_mins = ::mxx2::reduce::reduce_loc(timing##_durations, MPI_MINLOC, comm, 0); \
          auto dur_maxs = ::mxx2::reduce::reduce_loc(timing##_durations, MPI_MAXLOC, comm, 0); \
          auto dur_means = ::mxx2::reduce::reduce(timing##_durations, ::std::plus<double>(), comm, 0); \
          ::std::for_each(timing##_durations.begin(), timing##_durations.end(), [](double &x) { x = x*x; }); \
          auto dur_stdevs = ::mxx2::reduce::reduce(timing##_durations, ::std::plus<double>(), comm, 0); \
          \
          auto cnt_mins = ::mxx2::reduce::reduce_loc(timing##_counts, MPI_MINLOC, comm, 0); \
          auto cnt_maxs = ::mxx2::reduce::reduce_loc(timing##_counts, MPI_MAXLOC, comm, 0); \
          auto cnt_means = ::mxx2::reduce::reduce(timing##_counts, ::std::plus<double>(), comm, 0); \
          ::std::for_each(timing##_counts.begin(), timing##_counts.end(), [](double &x) { x = x*x; }); \
          auto cnt_stdevs = ::mxx2::reduce::reduce(timing##_counts, ::std::plus<double>(), comm, 0); \
          \
          if (rank == 0) { \
