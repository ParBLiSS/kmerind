...
public:
  typedef bliss::partition::range<std::size_t>  RangeType;
...
//        std::size_t chunkPos;    // for chunked iteration.  size since "data".
...
public:
   ...
   */
#if defined(USE_MPI)
  FileLoader(const std::string &_filename, const MPI_Comm& _comm,
      ...
      : file_handle(-1), filename(_filename), fileRange(), mmap_data(nullptr),
        ...
        chunkSize(_chunkSize), comm(_comm), dataBlocks(nullptr), recordSize(1)
  {
