...
//    if (!(count5 <= count && count <= (count5 + count3))) ERRORF("FAIL: number of successful inserts should be close to successful inserts without full buffers.");
  ...
  auto expectedFullMax = success / (bufferSize/data.length());
  auto expectedFullMin = success / (bufferSize/data.length()) - nthreads;

  if (fullBuffers.getSize() != sswap + fswap) ERRORF("FAIL: number of full Buffers do not match: fullbuffer size %lu  full count %lu + %lu", fullBuffers.getSize(), sswap, fswap);
  // buffer at 23 entries (86 bytes each, 2048 bytes per buffer) will not show as full until the next iterator.
  else if (((fswap + sswap) > expectedFullMax) || ((sswap + fswap) < expectedFullMin)) ERRORF("FAIL: number of full Buffers is not right: %lu+%lu should be between %lu and %lu", sswap, fswap, expectedFullMin, expectedFullMax);
  //else if (count4 != 0) ERRORF("FAIL: number of failed insert due to no buffer should be 0. actual %d", count4);
  else INFOF("PASS");
  INFOF("Number of failed attempt to append to buffer is %lu, success %lu. full buffers size: %lu.  success swapped = %lu, fail swapped = %lu.", failure, success, fullBuffers.getSize(), sswap, fswap);



  INFOF("TEST release: ");
  size_t sswap2 = 0, fswap2 = 0, error = 0, over = 0;
...
//        INFOF("%lu ", result.second);
  ...
  if (error != 0) ERRORF("FAIL: invalid argument exception during pop.  count = %lu", error);
  else if (over != 10) ERRORF("FAIL: failed on pop %lu times, expected 10", over);
  else if (fswap2 != 0) ERRORF("FAIL: succeeded in pop but not full buffer. %lu", fswap2);
  else if (sswap2 != (sswap + fswap)) ERRORF("FAIL: successful pops. expected %lu.  actual %lu", (sswap+fswap), sswap2);
  else
    INFOF("PASS");
  ...
  size_t success3 = 0, failure3 = 0, sswap3 = 0, fswap3 = 0, bytes3 = 0, chars3 = 0;
  size_t success4 = 0, bytes4 = 0;
  size_t gbytes = 0;
  //INFOF("full buffer: ");
  id = 0;
...
//        count7 = count1;  // save the number of successful inserts so far.
  ...
  if (bufferlt == bliss::concurrent::LockType::NONE && finals.size() != nthreads) ERRORF("FAIL: expected %lu threads have %lu actual.", nthreads, finals.size());
  ...
  if ((bytes3 + gbytes) != success3 * data.length() + bytes4) {
    ERRORF("FAIL: total bytes %lu (%lu + %lu).  expected %lu bytes (%lu + %lu).",
	 (bytes3 + gbytes), bytes3, gbytes, success3 * data.length() + bytes4, success3 * data.length(), bytes4);
  }

  //if (count7 != count/data.length()) ERRORF("FAIL: append count = %lu, actual data inserted is %ld", count7, count/data.length() );
  ...
  if (gbytes2 != 0) {
    ERRORF("FAIL: number of bytes STILL in message buffers is %lu",gbytes2);
  }
...
//
////  if (count1 != nelems) ERRORF("FAIL: number of successful inserts should be %lu.  actual %lu", nelems, count1);
////  else if (count2 != 0) ERRORF("FAIL: number of failed insert overall should be 0. actual %lu", count2);
////  else
//  if ((sswap3 + fswap3) != expectedFull && (fswap3 + sswap3) != expectedFull2) {
//    ERRORF("FAIL: number of full Buffers from failed insert is not right: %lu+%lu should be %lu or %lu", fswap3, sswap3, expectedFull, expectedFull2);
//    fflush(stdout);
//  }
//  else
    INFOF("PASS");
  ...
  INFOF("Number of failed attempt to append to buffer is %lu, success %lu. full buffers size: %lu.  success swapped = %lu, fail swapped = %lu. total bytes %lu + %lu", failure3, success3, fullBuffers.getSize(), sswap3, fswap3, bytes3, gbytes);
...
//        count7 = count1;  // save the number of successful inserts so far.
  ...
  if ((bytes + gbytes) != nelems * data.length()) {
    ERRORF("FAIL: total bytes %lu (%lu + %lu) for %ld entries.  expected %lu entries.", (bytes + gbytes), bytes, gbytes, (bytes + gbytes)/data.length(), nelems);
  }
  ...
  INFOF("Number appended to buffer is %lu, total attempts is %lu. full buffers size: %lu.  success swapped = %lu. total bytes %lu + %lu = %lu", nelems, attempts, fullBuffers.getSize(), swap, bytes, gbytes, (bytes + gbytes));
