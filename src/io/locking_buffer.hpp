      ...
      public:
        ...
        inline bool is_flushing() const {
          ...
          //std::atomic_thread_fence(std::memory_order_seq_cst);
        	...
        	return (curr_ptr == MAX_PTR_ADDR) && (writerCount > 0);
       ...
       // no chance of overflow with addition unless we have machines with 2B threads.  goal is to preserve sign bit during this op.
        ...
        inline typename std::enable_if<TS, uint8_t*>::type write_lock(const uint32_t count) {
          ...
          if (ptr == MAX_PTR_ADDR) {
          ...
          else if (ptr + count > max_ptr) {  // full buffer.  read lock it.
            curr_ptr = MAX_PTR_ADDR;
            ...
            if (ptr > max_ptr) {
              lock.unlock();
              return nullptr;
            }  //if ptr == max_ptr or just below, buffer must be FILLED in this cycle.  need to process it as if not full yet.
          } else {
            curr_ptr += count;
          }
        ...
        inline typename std::enable_if<!TS, uint8_t*>::type write_lock(const uint32_t count) {
          uint8_t* ptr = (uint8_t*)curr_ptr;
          if (ptr == MAX_PTR_ADDR) return nullptr;
          else if (ptr + count > max_ptr) {  // full buffer.  read lock it.
            curr_ptr = MAX_PTR_ADDR;
            if (end_ptr > ptr) end_ptr = ptr;
