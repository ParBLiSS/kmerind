 ...
 */
template<typename T>
class HashedVecMapTest : public ::testing::Test
{
  protected:
    ...
    { // generate some inputs
      ...
      test.insert(gold.begin(), gold.end());
    }
};

// indicate this is a typed test
TYPED_TEST_CASE_P(HashedVecMapTest);

TYPED_TEST_P(HashedVecMapTest, insert)
{
  bool same = false;


  ::fsc::hashed_vecmap<TypeParam, TypeParam > test2(this->gold.begin(), this->gold.end());
...
TYPED_TEST_P(HashedVecMapTest, equal_range)
{
  bool same = false;
    for (int i = 0; i < 99; ++i) {
...
TYPED_TEST_P(HashedVecMapTest, count)
{
    for (int i = 0; i < 99; ++i) {
...
TYPED_TEST_P(HashedVecMapTest, iterator)
{
...
TYPED_TEST_P(HashedVecMapTest, rand_iterator)
{
...
TYPED_TEST_P(HashedVecMapTest, copy)
{
  using valType = ::std::pair<TypeParam, TypeParam>;
...
REGISTER_TYPED_TEST_CASE_P(HashedVecMapTest, insert, equal_range, count, iterator, rand_iterator, copy);


//////////////////// RUN the tests with different types.

typedef ::testing::Types<int8_t, int16_t, int32_t,
    int64_t, uint64_t> HashedVecMapTestTypes;
INSTANTIATE_TYPED_TEST_CASE_P(Bliss, HashedVecMapTest, HashedVecMapTestTypes);
