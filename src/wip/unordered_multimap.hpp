...
namespace fsc {  // fast standard container
  ...
  class unordered_multimap {
    ...
    ::std::pair<subiter_type, subiter_type> equal_range_subcontainer(Key const & key) {
      auto iter = map.find(key);
    ...
    ::std::pair<const_subiter_type, const_subiter_type> equal_range_subcontainer(Key const & key) const {
      auto iter = map.find(key);
