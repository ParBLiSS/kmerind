...
TYPED_TEST_P(KmerTransformTest, trans_xor)
{
  ...
  for (size_t i = 0; i < this->iterations; ++i) {
    ...
    if (!local_same) {
      DEBUGF("ERROR: seq xor is not reversable with revcomp at iter %lu:\n\tkmer %s\n\trevcomp %s", i, km.toAlphabetString().c_str(), rev.toAlphabetString().c_str());
    }
    same &= local_same;

	local_same = (rev == op(std::make_pair(op(std::make_pair(rev, km)), km)));
    if (!local_same) {
      DEBUGF("ERROR: seq xor is not reversable with km at iter %lu:\n\tkmer %s\n\trevcomp %s", i, km.toAlphabetString().c_str(), rev.toAlphabetString().c_str());
    }
    same &= local_same;
    ...
    if (!local_same) {
      DEBUGF("ERROR: seq xor single vs double operands are not the same at iter %lu:\n\tkmer %s\n\trevcomp %s", i, km.toAlphabetString().c_str(), rev.toAlphabetString().c_str());
    }
    same &= local_same;
...
TYPED_TEST_P(KmerTransformTest, lex_greater)
{
  ...
  for (size_t i = 0; i < this->iterations; ++i) {
    ...
    local_same = (tmp == gold);
    if (!local_same) {
      DEBUGF("ERROR: seq lex_greater diff at iter %lu:\n\tinput %s\n\trev %s\n\tresult %s\n\tgold %s", i, km.toAlphabetString().c_str(), rev.toAlphabetString().c_str(), tmp.toAlphabetString().c_str(), gold.toAlphabetString().c_str());
    }
    same &= local_same;
